...
public class ComprobacionesItemProcessor implements ItemProcessor<Tiempo, Tiempo> {
    private static final Logger log = LoggerFactory.getLogger(ComprobacionesItemProcessor.class);
    ...
    public Tiempo process(Tiempo tiempo) {
        ...
        if (tiempo.getTemperatura() >= 50 || tiempo.getTemperatura() <= -20) {
            registrosFallidos++;
            try {
                writer.write(String.format("%s, %s, %d", tiempo.getLocalidad(), new SimpleDateFormat("dd/MM/yyyy").format(tiempo.getFecha()), tiempo.getTemperatura()));
                writer.newLine();
            } catch (IOException e) {
